...
### LLM 화자 문맥 보정 (실험적, 2026-08-29)

사용자 피드백: "화자분리가 음향(목소리)만 보고 문맥/상황을 전혀 이해하지 못한다." 맞는 지적이라, "제 이름은 OOO입니다" 같은 문맥 단서로 화자 병합을 제안하는 후처리 기능을 추가했습니다.

- **동작 방식**: 화자분리 결과를 AssemblyAI LLM Gateway에 보내 "문맥상 같은 사람인데 다른 화자 번호로 나뉜 경우"의 병합만 제안받음 (전체 텍스트를 다시 쓰게 하지 않고 라벨 매핑만 요청 — 환각 위험 최소화)
- **⚠️ 자동 적용 아님, 사람이 검토 후 선택 적용**: 검증 중 이 계정에서 접근 가능한 유일한 모델(`qwen3.5-4b-32k-fast`, 소형 모델 — Claude/GPT/Gemini는 이 계정 등급에서 403으로 막힘)이 (1) 명백히 다른 두 사람(등록 안내 스태프 vs 기술 얘기하는 참가자)을 병합하자고 제안하거나 (2) 입력에 없는 화자 라벨을 지어내는 것을 실제로 확인했습니다. 그래서:
...
- GUI: 전사 완료 후(화자분리 결과 + AssemblyAI 키 있을 때) "화자 병합 제안 받기 (LLM, 실험적)" 버튼 활성화
- 응답 속도가 2초~180초 타임아웃까지 들쭉날쭉해서 타임아웃을 넉넉히(180초) 잡아둠
