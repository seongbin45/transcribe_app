...
- **별개로 발견(버그 아님, 참고용 기록)**: 로컬 화자분리 단계 내부에서 pyannote가 segmentation → embeddings처럼 세부 단계를 넘어갈 때는 done/total이 다시 0부터 시작해 진행바 값이 실측상 33→100→50→100처럼 뒤로 튀는 지점이 실제로 존재함을 확인했다. 이는 `_on_transcribe_progress()` 코드에 이미 남겨져 있던 기존 설계상 제약("화자분리 내부 단계는 done/total이 단계마다 다시 0부터 시작해 신뢰할 수 없다")과 일치하는 동작이라 이번 리포트(단계 "전환" 시 이전 값이 얼어붙는 문제)와는 다른 사안으로 분류하고 고치지 않음 — 필요하면 별도 요청으로 다룰 것.
