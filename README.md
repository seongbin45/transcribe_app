...
- **제공자 2곳 지원, 자동 선택**: `.env`의 `GEMINI_API_KEY`가 있으면 Gemini를, 없으면 AssemblyAI LLM Gateway를 사용 ([core/llm_refine.py](transcribe_app/src/core/llm_refine.py)의 `pick_provider()`)
- **⚠️ 자동 적용 아님, 사람이 검토 후 선택 적용**: 처음 검증 때 AssemblyAI 계정에서 접근 가능한 유일한 모델(`qwen3.5-4b-32k-fast`, 소형 모델 — Claude/GPT/Gemini는 이 계정 등급에서 403으로 막힘)이 (1) 명백히 다른 두 사람(등록 안내 스태프 vs 기술 얘기하는 참가자)을 병합하자고 제안하거나 (2) 입력에 없는 화자 라벨을 지어내는 것을 실제로 확인했습니다. 그래서:
...
- **Gemini(`gemini-2.5-flash`)로 실데이터 재검증(2026-08-30)**: qwen보다 훨씬 신중한 판단을 확인.
...
- GUI: 전사 완료 후(화자분리 결과 + Gemini 또는 AssemblyAI 키 있을 때) "화자 병합 제안 받기 (LLM, 실험적)" 버튼 활성화
- 응답 속도: Gemini는 30~70초 정도로 비교적 일정, AssemblyAI(qwen)는 2초~180초 타임아웃까지 들쭉날쭉해서 타임아웃을 넉넉히(180초) 잡아둠
