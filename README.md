...
- **"파일로 저장" 버튼(추가 후속 요청, 2026-08-31)**: "전사가 끝났고(조건 충족) + 아직 저장하지 않았을 때"만 깜빡이도록 추가. `_on_transcribe_done()`에서 결과 세그먼트가 있으면 시작(빈 결과면 시작 안 함), `_on_export()`가 실제로 저장을 시도하는 순간 멈춤. 저장은 다른 버튼들과 달리 QThread 워커가 아니라 동기 호출이라 "시작 vs 완료" 구분이 뚜렷하지 않은데, 대신 세 갈래로 나눠 처리: 저장 다이얼로그를 **취소**하면(파일명을 안 고르면) 재개, 저장 중 **예외**가 나면 재개, **성공**하면 재개 안 함. 새 파일을 선택하면(이전 전사 결과가 무효가 되므로) 멈춤.
- 헤드리스 스모크 테스트로 네 버튼의 전체 상태 전이(시작→선택→클릭→실패 재개→성공 시 미재개→새 파일 선택 시 초기화)와 화자 수 검증 실패 시 버튼/진행바가 실제로 건드려지지 않는지, 저장 취소/실패/성공 세 갈래 모두 확인. 이 과정에서 테스트 스크립트 자체의 실수(저장 형식 콤보박스 기본값이 "Word(.docx)"인 걸 모르고 `export_txt`만 모킹해서 실패 시나리오가 실제로는 `export_docx`가 그대로 실행돼 성공해버리는 바람에 "재개돼야 하는데 안 됨" 오탐이 났던 것)를 발견해 텍스트 형식을 명시적으로 선택하도록 고쳐서 재검증함 — 앱 코드가 아니라 검증 스크립트의 버그였음. 테스트 중 `QMessageBox.critical()`/`warning()`이 오프스크린 환경에서 이벤트 루프 없이 모달로 블로킹되어 테스트가 멈추는 현상을 발견했는데, 이는 실제 앱 동작(모달이 사용자 클릭을 기다리는 건 정상)이 아니라 무한 대기형 헤드리스 테스트 스크립트 자체의 한계라 테스트에서만 무력화해서 확인함.
...
**버튼 자체 투명도 애니메이션 추가(실사용 스크린샷 피드백, 2026-08-31)**: 사용자가 실제 앱을 어두운 테마에서 캡처한 스크린샷을 보내며 "그림자가 아래에만 뜨고 버튼 자체가 투명해졌다가 투명도가 올라가는 깜빡임 로직이 없어서 잘 안 보인다"고 지적 — 바로 위 항목에서 그림자만 애니메이션하고 버튼의 입체 모양(그라디언트/테두리)은 고정해뒀던 절충이 실제로는 눈에 잘 안 띄었던 것.
- QSS는 여전히 프로퍼티 애니메이션 대상이 아니지만, "고정된 문자열"이 아니라 **매 애니메이션 프레임마다 알파 값을 넣어 새로 만든 QSS 문자열을 그때그때 다시 `setStyleSheet()`로 적용**하는 방식으로 우회(`_raised_style(alpha)` 함수가 배경 그라디언트/테두리/글자색을 전부 `rgba(...)`로 만듦). 그림자(`QGraphicsDropShadowEffect`)와 버튼 자체 색의 알파(70~255)를 같은 위상으로 동시에 오가게 해서, 그림자만이 아니라 **버튼 자체가 옅어졌다 진해지는 것**이 뚜렷하게 보이도록 함.
- **시각적 검증**: 사용자가 보낸 스크린샷과 같은 어두운 배경(`#1e1e1e`)에서 위상 0.0/0.5/1.0을 스크린샷으로 비교 — 0.0에서는 채도 낮은 갈색 톤으로 배경에 더 스며들고, 1.0에서는 선명한 주황/노랑 그라디언트로 확연히 도드라지는 것을 직접 확인. 회귀 테스트도 그림자(`blurRadius`)뿐 아니라 버튼 스타일 문자열의 알파 값(`rgba(255, 224, 130, 70)` → `rgba(255, 224, 130, 255)`)이 위상에 따라 실제로 바뀌는지까지 검증하도록 보강.
...
### 스피너 옆에 실행 전/실시간 예상 소요 시간 표시 (2026-08-31)

**요청**: "스피너 옆에 실시간 진행 예정 시간과 예상 시간을 적어두도록하세요. 따라서, 실행 전 예상시간을 판별하는 로직이 필요하고, 실행 중에는 실시간으로 변동을 감지해서 예상시간을 실시간으로 제공해줘야 합니다."

**설계**: [core/perf_profile.py](transcribe_app/src/core/perf_profile.py)(신규)가 두 가지를 함:
1. `estimate_seconds(stage, 오디오_길이, 모델_크기)` — 실행 전 예상 시간. "처리 속도"를 실시간 배율(RTF, real-time factor — 오디오 1초당 처리 시간)로 표현하고, 단계별(추출/STT/화자분리/API) RTF로 계산.
2. `record_actual(stage, 오디오_길이, 실제_걸린_시간, 모델_크기)` — 실행이 끝난 뒤 관찰된 RTF로 프로파일을 지수이동평균(EMA, α=0.3)으로 갱신, `perf_profile.json`(gitignore 대상, 사용자 기기별 값이라 커밋 안 함)에 저장. **자기보정**: 처음엔 대략적인 시드값을 쓰지만, 사용자가 실제로 실행할 때마다 "이 사용자의 실제 기기"에서 관찰된 속도로 점점 정확해짐.

**시드값의 정직한 근거**: large-v3 STT/화자분리의 초기 RTF는 이 프로젝트 검증 중 **같은 900초(15분) 오디오를 8번 실측한 평균**(STT 264.9~1640.4초, 화자분리 465.2~1090.6초로 최대 6배 넘게 편차 — 검증 세션 중 동시에 돌아간 다른 백그라운드 작업의 CPU 경합 때문, README "Day 2~4 확장 교차검증" 절 참고)에서 가져옴. 정밀한 벤치마크가 아니라 대략적인 출발점일 뿐임을 코드 주석에도 명시. small/medium 모델은 실측 데이터가 없어 whisper 공개 파라미터 수 비율(small 244M/medium 769M/large-v3 1550M)로 근사 — 방향성만 맞춘 값이고, 실행 후 각 모델별로 독립적으로 자기보정됨.

**실시간 갱신**: 신호의 "품질"에 따라 두 가지 방식을 섞어 씀:
- **STT 단계**: 처리한 윈도우 수/전체 윈도우 수가 뚝뚝 끊기지 않는 단일 카운터라, 진행 신호가 올 때마다 "지금까지 실제 걸린 시간 ÷ 지금까지 진행률"로 전체 예상 시간을 다시 계산(`_EtaTracker.on_progress`) — 진짜 실시간 변동 감지.
- **화자분리/추출/API 단계**: pyannote 내부 단계(segmentation/embeddings/...)는 done/total이 단계마다 다시 0부터 시작해 신뢰할 수 없고, 추출/API는 애초에 진행 신호가 없음 — 대신 사전 예상치에서 경과 시간을 빼는 카운트다운을 1초마다 갱신(그래도 "실시간으로 매초 갱신된다"는 점은 동일).
- `gui/main_window.py`의 `TranscribeWorker`에 `stage_started(stage, 예상시간)`/`stage_done(stage, 실제시간)` 시그널 추가, `MainWindow`가 이걸 받아 `_EtaTracker`를 단계별로 새로 만들고 `record_actual()`로 결과를 기록. 스피너(진행바) 옆에 `eta_label`로 "전사(STT) 예상: 약 32초 남음 (경과 14초)"처럼 표시.

**실제 검증(중요 발견 포함)**:
- 실제 3분 클립(`personal_probe.wav`, small 모델)으로 전체 파이프라인을 실행해 STT 단계의 예상 시간이 실제로 32초 → 1분22초 → 56초 → 41초 → 30초 → 35초 → 16초 → "곧 완료..."로 **실시간으로 변동하며 갱신**되는 것을 직접 확인 — 윈도우마다 처리 시간이 달라서 추정치가 오르내리는 것도 실제 현상(설계상 자연스러움).
- 화자분리 단계 진입 시 "약 2분41초 남음"으로 새로 시작했고, 실제로는 더 오래 걸렸음(관찰된 RTF 1.13 vs 시드 0.894) — 완료 후 `perf_profile.json`에 반영되어 다음 실행부터는 더 정확해짐을 실제 파일로 확인.
...
### 설정 창 구조 개편: 카테고리 목록 + 옆 패널 (2026-08-31)

**요청**: "옵션을 클릭하다 보면, 설정 메뉴가 너무 길어진다. 설정 창의 위 아래 폭 제한을 두고 옵션 선택이 생기면 옆에서 보이도록 바꿔라." — Groq+pyannoteAI 엔진 추가로 API 키 입력칸이 늘어나면서 특히 심해짐(엔진 라디오를 바꿀 때마다 키 입력 그룹이 1개↔2개로 바뀌며 창이 위아래로 계속 늘었다 줄었다 함).

**설계**: [gui/widgets/settings_dialog.py](transcribe_app/src/gui/widgets/settings_dialog.py)를 "환경설정" 창에서 흔한 마스터-디테일(왼쪽 카테고리 목록 + 오른쪽 내용 패널) 구조로 다시 짬 — 언어 / 엔진·API 키 / LLM·고급 3개 카테고리로 나누고, `QDialog.setFixedSize()`로 창 크기를 고정. 엔진 카테고리 안에서도 라디오(왼쪽)와 그 라디오에 맞는 API 키 입력칸(오른쪽, `QStackedWidget`)을 나란히 둬서, 라디오를 바꿔도 키 입력칸이 "옆" 자리 안에서만 바뀌고 창이 늘어나지 않게 함.

**실제로 두 번 잘못 잡았다가 스크린샷으로 발견해 고친 폭 문제**:
1. 처음엔 라디오 버튼 라벨에 줄바꿈(`\n`)을 넣어 3줄로 만들었는데, `QRadioButton`이 자동으로 줄바꿈을 하지 않고 **가장 긴 줄 기준으로 폭을 요구**해서 "STT 엔진" 그룹의 `sizeHint`가 528px까지 벌어졌고, 그 바람에 오른쪽 API 키 패널이 창 밖으로 밀려 잘려 보였음(실제 스크린샷으로 "저장된 키 삭제" 버튼이 잘린 것을 확인). 라벨을 한 줄로 줄이고 설명은 툴팁으로 옮겨서 해결.
2. 그렇게 고친 뒤에도 여전히 폭이 넉넉지 않았는데, 원인을 더 파보니 **로컬 모델 크기 콤보박스**가 범인이었음 — 항목 중 `"large-v3 (권장, 느림·최초 실행 시 다운로드)"`가 길어서 콤보박스의 `sizeHint`가 390px나 요구하고 있었음(`dlg.model_combo.sizeHint()`로 직접 측정해서 확인). `setMaximumWidth(190)`로 닫힌 상태 폭을 좁게 고정(펼치면 목록엔 전체 텍스트가 그대로 보임)하고, 잘릴 수 있는 텍스트는 툴팁으로 보완.
3. 두 원인을 고친 뒤 실측 `sizeHint`(STT 엔진 그룹 324px + API 키 패널 340px + 카테고리 목록 130px + 여백)에 맞춰 창 크기를 860×460으로 최종 확정.
...
### 진행바가 pyannoteAI 화자분리 단계에서 100%에 멈춰 보이는 문제 수정 (2026-08-31, 실사용 리포트)

**리포트**: Groq+pyannoteAI 조합으로 실제 파일(10시간 넘는 개인 녹음)을 돌린 화면 캡처에서, Groq STT 단계(0~100%)가 끝난 뒤 pyannoteAI 화자분리 단계로 넘어갔는데도 진행바가 100%에 그대로 멈춰 있고, 상태 표시줄엔 "pyannoteAI 화자분리 중: 업로드 중"이라고 떠서 겉보기엔 다 끝난 것처럼 보이는데 실제로는 이제 막 시작한 것이었음.

**원인**: pyannoteAI는 폴링 응답에 실제 백분율을 안 줘서([core/engines/pyannoteai_engine.py](transcribe_app/src/core/engines/pyannoteai_engine.py)), [gui/main_window.py](transcribe_app/src/gui/main_window.py)의 해당 단계 진행 콜백은 의도적으로 `progress` 시그널을 아예 보내지 않고 상태 문구만 갱신하도록 만들어뒀었다(대신 남은 시간은 ETA 카운트다운이 담당). 문제는 진행바(`QProgressBar`) 값 자체를 리셋하는 코드가 어디에도 없어서, 직전 단계(Groq STT)가 마지막으로 `setValue(100)`을 해둔 값이 그대로 얼어붙어 남아있었던 것.

**수정**: `MainWindow._on_stage_started()`(각 단계가 시작될 때마다 호출됨)에서 진행바를 매번 불확정(스피너, `setRange(0, 0)`) 모드로 먼저 리셋하도록 변경. 실제 퍼센트가 들어오는 단계(로컬 STT/화자분리, Groq STT)는 `_on_transcribe_progress()`가 첫 진행 신호를 받는 즉시 다시 확정(0~100%) 모드로 자동 전환하므로 기존 동작 그대로고, pyannoteAI처럼 퍼센트가 아예 없는 단계만 스피너로 정직하게 보인다.

**검증**: Groq/pyannoteAI 엔진을 모의(mock)로 교체해 `MainWindow._on_transcribe()` 전체 경로를 재생 — 수정 전 코드로 먼저 돌려서 "pyannoteai_diarize 단계 시작 시점에 진행바가 `max=100, value=100`으로 얼어붙어 있다"는 리포트된 증상을 실제로 재현시켜 테스트가 버그를 잡아낼 수 있음을 확인한 뒤, 수정 후 코드로 다시 돌려 같은 시점에 `max=0`(스피너 모드)으로 정확히 리셋되는 것을 확인. Groq STT 단계가 실제로 100% 확정 모드까지 도달한 뒤라는 것도 함께 검증해, "애초에 확정 모드로 안 갔다"가 아니라 "리셋이 안 됐다"는 원인을 정확히 짚었다.

**후속 교차검증(같은 날)**: 이 수정이 pyannoteAI 조합에만 우연히 맞아떨어진 게 아니라 모든 엔진 조합에서 일관되게 동작하는지 확인하기 위해, 실제 `MainWindow`/`TranscribeWorker` 신호 배선은 그대로 두고 엔진만 모의로 바꿔 7가지 시나리오(로컬 CPU 화자분리 켜짐/꺼짐, 로컬 GPU(모의) 화자분리 켜짐, AssemblyAI 화자분리 켜짐/꺼짐, Groq+pyannoteAI 화자분리 켜짐, Groq만)를 전부 재생:
- 모든 시나리오, 모든 단계 시작 시점에서 진행바가 예외 없이 스피너로 리셋됨을 확인(이전 단계 값이 새는 경로가 더는 없음).
- 실제 진행률을 주는 단계(로컬/GPU STT·화자분리, Groq STT)는 확정 모드로 정상 전환되어 100%까지 도달.
- 진행률을 아예 안 주는 단계(AssemblyAI 전체, pyannoteAI 화자분리)는 끝까지 스피너 상태로 남아있는 게 오히려 정직한 표시라 정상 처리.
- 화자분리를 껐을 때는 단계가 하나만(로컬/Groq는 stt만, AssemblyAI는 api 하나로 이미 diarize 포함) 발생하고 곧바로 완료 처리됨을 확인.
- **별개로 발견(참고용 기록, 아래에서 별도로 수정함)**: 로컬 화자분리 단계 내부에서 pyannote가 segmentation → embeddings처럼 세부 단계를 넘어갈 때는 done/total이 다시 0부터 시작해 진행바 값이 실측상 33→100→50→100처럼 뒤로 튀는 지점이 실제로 존재함을 확인했다.

### 로컬 화자분리 진행바가 내부 단계 전환 시 뒤로 튀는 문제 수정 (2026-08-31, 사용자 요청)

**증상**: 위 교차검증에서 발견한 것 — 로컬(또는 GPU) 화자분리 단계 안에서 pyannote가 segmentation → embeddings 등으로 내부 단계를 넘어갈 때마다 [core/engines/local_pyannote.py](transcribe_app/src/core/engines/local_pyannote.py)의 hook이 그 단계 자신의 done/total(0부터 다시 시작)을 그대로 넘겨서, 진행바가 예를 들어 segmentation 100% → embeddings 50%처럼 눈에 보이게 뒤로 튀었다.

**수정**: [main_window.py](transcribe_app/src/gui/main_window.py)에 단계별 "워터마크"(`_progress_watermark`, 현재 단계에서 지금까지 보여준 최댓값)를 추가 — `_on_transcribe_progress()`에서 새로 계산한 퍼센트를 워터마크와 비교해 더 큰 값만 실제로 표시하고(`pct = max(pct, self._progress_watermark)`), 워터마크는 매 단계 시작(`_on_stage_started`)마다 0으로 리셋한다(그래야 stt 100% 다음 diarize가 정상적으로 0%부터 다시 채워짐). **트레이드오프를 정직하게 밝히면**: 이 방식은 절대 뒤로 가지 않는 대신, 나중 내부 단계의 비중이 이전 단계보다 작으면(예: embeddings가 segmentation보다 빨리 끝나는 경우) 진행바가 실제 완료 시점보다 먼저 100%를 보여줄 수 있다 — 그래도 상태 표시줄 문구와 ETA 카운트다운은 실제 내부 단계 이름/경과 시간을 그대로 반영하므로, "다 끝났는데 안 끝난 것처럼 보이는" 원래 리포트보다 "약간 일찍 100%로 보이는" 쪽이 사용자 경험상 낫다고 판단했다(뒤로 튀는 것은 어떤 경우에도 정상적인 진행바 동작으로 보이지 않기 때문).

**검증**: 수정 전 코드로 먼저 돌려서 로컬 화자분리(CPU/GPU 둘 다) 진행바 값이 실제로 `[33, 100, 50, 100]`처럼 뒤로 튀는 걸 재현해 테스트가 이 회귀를 잡아낼 수 있음을 확인한 뒤, 수정 후 코드로 다시 돌려 같은 시나리오에서 `[33, 100, 100, 100]`으로 절대 감소하지 않는 것을 확인. 앞서 만든 7가지 엔진 조합 교차검증 스위트를 그대로 재실행해 이 변경이 다른 시나리오(단계 전환 스피너 리셋 등)에 회귀를 만들지 않는 것도 함께 확인했다.

### pyannoteAI 폴링 중 read timeout으로 장시간 작업 전체가 실패하는 문제 수정 (2026-08-31, 실사용 오류 리포트)

**리포트**: 10시간 넘는 실제 파일을 Groq+pyannoteAI로 돌리던 중 `전사 중 오류: HTTPSConnectionPool(host='api.pyannote.ai', port=443): Read timed out. (read timeout=30)` 오류로 작업 전체가 중단됨.

**원인**: [core/engines/pyannoteai_engine.py](transcribe_app/src/core/engines/pyannoteai_engine.py)의 화자분리 대기 폴링은 `POLL_INTERVAL_SEC=3`초 간격으로 최대 1시간(`POLL_TIMEOUT_SEC=3600`)까지, 즉 최대 1200번 `GET /v1/jobs/{jobId}`를 반복 호출한다. 그런데 각 호출마다 재시도 없이 단 한 번이라도 `read timeout=30`(또는 연결 끊김)이 나면 그 순간 예외가 그대로 위로 튀어서, 몇 시간째 잘 진행되고 있던 작업 전체가 통째로 실패 처리됐다 — 업로드/화자분리 요청 POST도 동일하게 재시도 로직이 전혀 없었다.

**수정**: `_request_with_retries()` 헬퍼를 추가해 `requests.exceptions.Timeout`/`ConnectionError`(서버가 아예 응답을 못 준 경우)에 한해서만 지수 백오프(3초, 6초, 9초, 12초 간격)로 최대 5회까지 재시도하도록 [media/input 업로드 URL 요청], [오디오 PUT 업로드](파일 핸들을 다시 열어야 해서 별도 재시도 루프), [diarize 요청], [잡 상태 폴링] 네 곳 모두에 적용. **서버가 실제로 4xx/5xx를 준 경우는 재시도해도 결과가 바뀌지 않으므로 의도적으로 재시도하지 않고 기존처럼 즉시 실패 처리**한다(재시도 대상을 "응답이 아예 안 온 경우"로만 좁힘). 재시도 중에는 상태 표시줄에 "네트워크 오류, N초 후 재시도 중 (1/5회)"처럼 그대로 보여서, 멈춘 것처럼 보이지 않게 했다. 폴링 요청 자체의 타임아웃도 30초 → 45초로 늘려 여유를 뒀다.

**검증**: pyannoteAI 서버 응답을 모의(mock)로 바꿔 세 가지를 확인 — (1) 실제 오류 메시지와 동일한 `ReadTimeout`을 2번 발생시킨 뒤 3번째 시도에서 성공하면 재시도 상태 메시지 2개를 거쳐 정상적으로 결과를 받아오는지, (2) 5번을 계속 실패시키면 "네트워크 오류로 5회 재시도 후에도 실패했습니다"라는 명확한 한국어 오류로 포기하는지, (3) 서버가 실제 400 응답을 준 경우엔(재시도해도 소용없는 경우) 재시도 없이 정확히 1번만 호출하고 바로 실패하는지 — 셋 다 통과 확인.

**후속 교차검증(재시도 중 진행바/스피너 동작, 2026-08-31, 사용자 요청)**: 재시도 로직 자체는 통과했지만, 그동안 GUI 진행바가 실제로 어떻게 보이는지는 별도로 확인이 필요해서 `MainWindow`/`TranscribeWorker`를 실제 그대로 두고 `requests` 계층만 모의로 바꿔 재시도를 실제로 발생시키며 두 시나리오를 추가로 검증:
- **재시도 후 성공**: read timeout을 2번 겪고 3번째 시도에서 성공하는 경우, 재시도 메시지("네트워크 오류, N초 후 재시도 중")가 실제로 상태 표시줄까지 전달되고, 그 순간들 모두 진행바가 스피너(불확정) 상태를 유지하며(`max=0`, 항상 보임) — 얼어붙은 값이 잘못 표시되는 일 없이 — 재시도 이후 정상적으로 완료(진행바 숨김, ETA 정리, 화자분리 결과 반영)까지 이어지는 것을 확인.
- **재시도 5회를 모두 소진해 실패**: 재시도 메시지가 정확히 4개(총 5번 시도 중 마지막 시도 직전까지) 뜨고, 그 동안에도 진행바는 계속 스피너 상태를 유지하다가, 최종 실패 처리 시점(`_on_transcribe_failed`)에 진행바가 정확히 숨겨지고 "전사 시작" 버튼이 다시 활성화되는 것을 확인 — 재시도 중이던 상태가 얼어붙은 채로 남지 않음.
- 참고: 진행바가 재시도 중에도 문제없이 동작하는 이유는 애초에 pyannoteAI 화자분리 단계 전체가 실제 퍼센트를 안 받는 스피너(불확정) 모드로 고정되어 있고(`_dia_progress`가 `progress` 시그널 자체를 안 보냄, 위 절 참고) 재시도 메시지도 같은 경로(`status` 시그널)로만 전달되기 때문 — 재시도가 진행바 값 자체를 건드릴 여지가 처음부터 없다.
